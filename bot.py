...
starttext1 = open("start.txt", 'r', encoding="utf8")
starttext = starttext1.readline()
starttext = starttext + starttext1.readline()
misatoMap = starttext1.readline(3)
...
async def start_comms(message: types.Message):
    ...
    await message.answer(starttext, reply_markup=keyboard)
...
async def text_plus1 (call: types.CallbackQuery, floor=floor):
    ...
    keyboard = types.InlineKeyboardMarkup(row_width=2)
    keyboard.add(*buttons)
    await call.message.edit_text("Этаж "+ str(floor),misatoMap, reply_markup=keyboard)
...
async def text_plus1 (call: types.CallbackQuery, floor=floor):
    ...
    await call.message.edit_text(f"Этаж + {str(floor)},{misatoMap}", reply_markup=keyboard)
